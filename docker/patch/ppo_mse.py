...
def _add_log_prob_and_value_to_episodes_recurrent(
    ...
    with torch.no_grad(), pfrl.utils.evaluating(model):
        ...
        (flat_distribs, flat_vs), _ , mask_attention = pack_and_forward(model, seqs_states, rs)
        (_, flat_next_vs), _, _ = pack_and_forward(model, seqs_next_states, next_rs)
...
def _make_dataset_recurrent(
    ...
    mask_attention = _add_log_prob_and_value_to_episodes_recurrent(
        episodes=episodes,
        model=model,
        phi=phi,
...
class PPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update_if_dataset_is_ready(self):
        ...
        if dataset_size >= self.update_interval:
            ...
            if self.recurrent:
                dataset, mask_attention = _make_dataset_recurrent(
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(
            ...
            mask_v, mask_a = self.mask_attention
            mask_v = mask_v.to(torch.float).to('cuda')
            mask_a = mask_a.to(torch.float).to('cuda')
    ...
    def _update_once_recurrent(self, episodes, mean_advs, std_advs):
        ...
        (flat_distribs, flat_vs_pred), _, mask_attention = pack_and_forward(self.model, seqs_states, rs)
    ...
    def _lossfun(
        self, entropy, vs_pred, log_probs, vs_pred_old, log_probs_old, advs, vs_teacher, mask_v, mask_a
    ):
        ...
        mask_blue = torch.zeros(128,1,16,16, requires_grad=True).to(torch.float).to('cuda')
        mask_red = torch.ones(128,1,16,16, requires_grad=True).to(torch.float).to('cuda')

        mask_attention_loss_v = F.mse_loss(mask_v, mask_blue)
        mask_attention_loss_a = F.mse_loss(mask_a, mask_blue)
        ...
        loss = (
            ...
            + 0.01*(mask_attention_loss_v + mask_attention_loss_a)
        )
