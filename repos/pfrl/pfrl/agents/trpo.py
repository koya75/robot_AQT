...
from repos.pfrl.pfrl.agents.ppo import (  # NOQA
...
class TRPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        if recurrent:
            self.model = pf.nn.RecurrentBranched(policy, vf)
        else:
            self.model = pf.nn.Branched(policy, vf)
    ...
    def _compute_kl_constrained_step(self, action_distrib, action_distrib_old, gain):
        ...
        step_direction = pf.utils.conjugate_gradient(
