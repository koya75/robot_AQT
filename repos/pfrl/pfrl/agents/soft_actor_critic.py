...
from repos.pfrl.pfrl.agent import AttributeSavingMixin, BatchAgent
from repos.pfrl.pfrl.replay_buffer import ReplayUpdater, batch_experiences
from repos.pfrl.pfrl.utils import clip_l2_grad_norm_
...
class SoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def update_q_func(self, batch):
        ...
        with torch.no_grad(), pf.utils.evaluating(self.policy), pf.utils.evaluating(
            self.target_q_func1
        ), pf.utils.evaluating(self.target_q_func2):
    ...
    def batch_select_greedy_action(self, batch_obs, deterministic=False):
        with torch.no_grad(),pf.utils.evaluating(self.policy):
